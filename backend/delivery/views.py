...
class DeliveryListView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        elif hasattr(user, 'customer_orders'):
            return Delivery.objects.filter(order__customer=user)
        elif hasattr(user, 'vendor_products'):
...
class DeliveryStatusView(UpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if self.request.user != delivery.delivery_person and \
           ...
           self.request.user.role not in ['admin', 'staff']:
...
class DeliveryDashboardView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            profile, created = DeliveryPersonProfile.objects.get_or_create(
                user=user,
                defaults={
            ...
            logger.error(f"Error getting/creating delivery profile: {str(e)}")
            return Response(
                {'error': 'Failed to access delivery profile'},
        ...
        # Get active deliveries with order details
        ...
        for delivery in active_deliveries:
            active_deliveries_data.append({
                'id': delivery.id,
                'customer': f"{delivery.order.customer.first_name} {delivery.order.customer.last_name}",
                'address': delivery.order.delivery_address or 'Address not provided',
                'amount': f"${delivery.delivery_fee}",
                'time': delivery.assigned_at.strftime('%H:%M'),
...
class AvailableDeliveriesView(ListAPIView):
    """
    List all available orders that need delivery
    """
    serializer_class = DeliverySerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        # The signal now handles creation of Delivery objects.
        # This view just lists available deliveries.
        return Delivery.objects.filter(
            status='pending',
            delivery_person__isnull=True
        ).select_related('order', 'order__customer').prefetch_related('order__items')
...
class AcceptOrderView(APIView):
    ...
    def post(self, request, order_id, *args, **kwargs):
        ...
        if not hasattr(delivery_person, 'delivery_profile'):
            return Response(
                {'error': 'Only delivery persons can accept orders.'},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        try:
            with transaction.atomic():
                ...
                if order.status != 'order_placed' or order.delivery_person is not None:
                    return Response(
                        {'message': 'Order has already been taken.'},
                        status=status.HTTP_409_CONFLICT
                    )

                # Find the corresponding delivery object created by the signal
                delivery = get_object_or_404(Delivery, order=order)
                if delivery.delivery_person is not None:
                     return Response(
                        {'message': 'Order has already been taken by another person.'},
                        status=status.HTTP_409_CONFLICT
                    )

                # Assign the delivery person to the order and the delivery
    ...
    Your order has been assigned to {delivery_person.first_name} {delivery_person.last_name}
    Track your order in the CampusConnect app
    Thank you for choosing us!
    """
                    try:
                        notification_service.send_whatsapp(
                            "+254790010567",  # Test number
                            message.strip()
                        )
                    except Exception as e:
                        logger.error(f"Failed to send WhatsApp confirmation for order {order.id}: {e}")
            ...
            # This should not happen if the signal is working correctly
            return Response(
